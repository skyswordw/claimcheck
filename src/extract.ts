...
export function extractClaims(text: string): Claim[] {
  ...
  const claim = (
    ...
    extra: { value?: number; unit?: string; issue?: number; raw?: string } = {}
  ): void => {
    if (isNegated(m.index ?? 0)) return;
    const c: Claim = { kind, raw: (extra.raw ?? m[0]).trim(), verifiability };
  ...
  for (const m of text.matchAll(/\b(fix(?:e[sd])?|close[sd]?|resolve[sd]?)\s+(#\d+(?:[ \t,]*(?:and[ \t]+|&[ \t]*)?#\d+)*)/gi)) {
    ...
    for (const im of (m[2] as string).matchAll(/#(\d+)/g)) {
  ...
  for (const m of text.matchAll(/\b(\d+(?:\.\d+)?)\s*(%|x)\s*(?:faster|slower|speed-?up|quicker)\b/gi)) {
    claim("perf", m, "reproduction", { value: Number(m[1]), unit: (m[2] as string).toLowerCase() });
  }
